---
name: Robert
surname: Wiederstein
position: "Developer • Programmer"
address: "214 Sycamore St. Unit 402, Evansville, IN 47708"
#phone: 270.869.5112
www: robwiederstein.org
email: "rob@robwiederstein.org"
twitter: RobWiederstein
github: RobWiederstein
#linkedin: <a href='https://www.linkedin.com/in/rob-wiederstein-797553138/'>rob-wiederstein</a>
date: "`r format(Sys.time(), '%B %Y')`"
output: vitae::awesomecv
headcolor: 3026ef
profilepic: pic.jpg
csl: numeric-apa-cv.csl
docname: CV/Resume
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)
library(vitae)
library(glue)
library(myCV)
```

# Summary

I have been programming in R part-time for 5 years, and full time for the last two years.  My experiences and interests are diverse and set out  on my [**blog**](https://www.robwiederstein.org) and GitHub [**account**](https://github.com/RobWiederstein). I enjoy traveling, reading, guitar and cooking family meals.


```{r load-data, include=FALSE}
jobs <- readr::read_csv("../data/jobs.csv")
schools <- readr::read_csv("../data/schools.csv")
skills <- readr::read_csv("../data/skills.csv")
```

# Education

```{r education, echo=FALSE}
detailed_entries(schools, 
                 when = glue("{start_date}--{end_date}"),
                 what = degree,
                 where = loc,
                 with = uni)
```

# Work

```{r work}
jobs %>% 
  detailed_entries(
    what = role,
    when = glue("{start_date}--{end_date}"),
    with = company,
    where = where,
    why = details)
```
# Publications

```{r Publications}
bibliography_entries("pubs.bib")
```

# Selected Presentations

```{r Presentations}
# Normal articles excluding editorials
bibliography_entries("talks.bib")
```

# Skills

```{r skills}
skills %>%
    vitae::detailed_entries(
        with = familiar,
        what = skills
    )
```

# Application Software

```{r software, eval=T}
tribble(
  ~ pkg, ~ when, ~ role, ~ details,
  'retirementDash', '2021', 'Developer/author', 'A dashboard of retirement locations filtered by health, environment, housing affordability, and demographic criteria. Inter-county migration data are also included.',
  'retirementData', '2021', 'Developer/author', 'The dataset provides demographic, economic, environmental, and health data by county to determine retirement location. All United States counties are included.',
  'KeyWestMopeds', '2021', 'Developer/author', 'The package explores motorcycle crash data from 2012 through 2021 furnished via the Florida Highway Safety and Motor Vehicle Department through its open data portal.'
) %>% 
  brief_entries(glue::glue("\\textbf{<pkg>}: <role> \\newline <details>", .open = "<", .close = ">"), paste0(when, "\\newline"), .protect = FALSE)

```

# Packages Used

```{r}
library(dplyr)
knitr::write_bib(c("vitae", "tibble", "glue", "myCV"), "packages.bib")

bibliography_entries("packages.bib") %>%
  arrange(desc(author$family), issued)
```

